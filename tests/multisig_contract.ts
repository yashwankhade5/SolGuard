...
describe("multisig_contract", () => {
 ...
 const [vaultPda, vaultbump] = anchor.web3.PublicKey.findProgramAddressSync(
  ...
  it("proposal approval", async () => {
    // Add your test here.
    const tx = await program.methods.approvaProposal(new anchor.BN(0)).accountsPartial({signer:wallet1.publicKey,
      multisigConfig:multiSigPda,
      proposal:proposalPda
...
it("propsal execution", async () => {
    // Add your test here.
    const tx = await program.methods.executeProposal(new anchor.BN(0)).accountsPartial({signer:wallet1.publicKey,
